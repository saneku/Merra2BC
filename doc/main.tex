...
\includegraphics[width=\linewidth]{repo_logo.png}
\end{minipage}
\hfill
\begin{minipage}[t]{0.80\textwidth}
{\LARGE\bfseries Merra2BC User manual\par}
{\large Alexander Ukhov, KAUST\par}
{\large 1 March, 2026\par}
\end{minipage}
\vspace{0.6cm}
...
The workflow of the {Merra2BC} interpolator is shown in Fig.~\ref{fig:repo_thumbnail}. The utility is designed to be flexible and can be easily modified to work with different WRF-Chem chemical mechanisms and aerosol modules.
{Merra2BC} is written in Python. The utility requires additional modules that need to be installed in the Python environment: NumPy (numpy), NetCDF4 (netcdf4) interface to work with NetCDF files, and SciPy (scipy) package.

\subsection{Data requirements}
The required MERRA-2 reanalysis data set including aerosol and gaseous collections is publicly available online:
\begin{itemize}
\item \url{https://disc.gsfc.nasa.gov/datasets/M2I3NVAER_5.12.4/summary}
\item \url{https://disc.gsfc.nasa.gov/datasets/M2I3NVCHM_5.12.4/summary}
\end{itemize}
Depending on the requirements, all or one of the following aerosol and gaseous collections need to be downloaded:
...
\begin{figure}[h!]
\centering
% Source artwork lives in repo_thumbnail.svg; PNG is used for portable LaTeX rendering.
\includegraphics[width=\textwidth]{repo_thumbnail.png}
\caption{Merra2BC workflow overview from MERRA-2 inputs to updated WRF-Chem initial and boundary-condition files.}
\label{fig:repo_thumbnail}
\end{figure}
...
The interpolation procedure is based on the bivariate spline approximation for horizontal interpolation and linear interpolation for vertical interpolation.
...
\begin{enumerate}
\item Run \textit{real.exe} with \textit{chem\_in\_opt = 0}, which will produce initial \textit{wrfinput\_d01} and boundary conditions \textit{wrfbdy\_d01} files required by the WRF-Chem simulation.
\item Download required MERRA-2 files from \url{https://disc.gsfc.nasa.gov/datasets/M2I3NVAER_5.12.4/summary} and \url{https://disc.gsfc.nasa.gov/datasets/M2I3NVCHM_5.12.4/summary};
\item Download {Merra2BC} from \url{https://github.com/saneku/Merra2BC}.
\item Set defaults in \textit{src/config.py} file, which contains
\begin{itemize}
...
\end{itemize}
\item Runtime arguments can override configuration values without editing \textit{src/config.py}:
\texttt{--wrf\_input\_file}, \texttt{--wrf\_bdy\_file},
\texttt{--wrf\_met\_files}, \texttt{--merra2\_files},
\texttt{--do\_IC=true|false}, \texttt{--do\_BC=true|false}, and
\texttt{--mapping\_profile}. Use ``python \textit{main.py} --help'' to print all options.
Quote masks passed to \texttt{--wrf\_met\_files} and \texttt{--merra2\_files} to avoid shell expansion.
Use one MERRA-2 collection per run (\textit{inst3\_3d\_aer\_Nv} or \textit{inst3\_3d\_chm\_Nv}) consistent with active \textit{spc\_map}.
\item \textit{real.exe} sets default boundary and initial conditions for some chemical species.  {Merra2BC} adds interpolated values to the existing values, which may cause incorrect concentration values. To avoid this, run ``python \textit{zero\_fields.py} --do\_IC=true --do\_BC=true'', which will zero the required fields.
\item Run ``python \textit{main.py}'', which will do the interpolation. As a result, files \textit{wrfinput\_d01} and \textit{wrfbdy\_d01} will be updated by interpolated MERRA-2 values. Example:
``python \textit{main.py} --wrf\_input\_file /path/to/wrfinput\_d01 --wrf\_bdy\_file /path/to/wrfbdy\_d01 --wrf\_met\_files '/path/to/met\_em.d01.*.nc' --merra2\_files '/path/to/MERRA2\_400.inst3\_3d\_aer\_Nv.2025*.nc4' --mapping\_profile maps/wrfchem\_aer\_so2\_dust1.map --do\_IC=true --do\_BC=true''.
Constant map entries are written directly in output-variable units. For example, WRF-Chem can use \texttt{co2 -> 400.0;1.0} for ppmv, while MPAS-A kg\,kg$^{-1}$ constants must be converted before being placed in the map. For WRF-Chem boundary conditions, corresponding tendency fields are set to zero.
\item Modify the WRF-Chem \textit{namelist.input} file at section \textit{\&chem}: set \textit{have\_bcs\_chem = .true.} to activate updated boundary conditions and \textit{chem\_in\_opt = 1} to preserve updated initial conditions.
\item Run \textit{wrf.exe}.
\end{enumerate}

\hack{\newpage}
\begin{thebibliography}{99}
\bibitem[Randles et~al.(2017)Randles, da Silva, Buchard, Colarco, Darmenov, Govindaraju, Smirnov, Holben, Ferrare, Hair, et~al.]{randles2017merra}
Randles, C.~A., da Silva, A.~M., Buchard, V., Colarco, P.~R., Darmenov, A.,
Govindaraju, R., Smirnov, A., Holben, B., Ferrare, R., Hair, J., and others.
(2017).
The MERRA-2 aerosol reanalysis, 1980 onward. Part I: System description and
data assimilation evaluation.
\textit{Journal of Climate}, 30(17), 6823--6850.

\bibitem[Bosilovich et~al.(2016)Bosilovich, Lucchesi, and Suarez]{bosilovich2016merra}
Bosilovich, M.~G., Lucchesi, R., and Suarez, M. (2016).
