...
\texttt{--init\_co2\_ch4=true|false}. Use ``python \textit{main.py} --help'' to print all options.
...
``python \textit{main.py} --wrf\_input\_file /path/to/wrfinput\_d01 --wrf\_bdy\_file /path/to/wrfbdy\_d01 --wrf\_met\_files '/path/to/met\_em.d01.*.nc' --merra2\_files '/path/to/MERRA2\_400.inst3\_3d\_aer\_Nv.2025*.nc4' --do\_IC=true --do\_BC=true --init\_co2\_ch4=false''.
If \texttt{--init\_co2\_ch4=true} is used, \textit{co2} and \textit{ch4} are set to fixed values (400 and 1.7\,\unit{ppmv}, respectively) in IC/BC, and corresponding BC tendencies are set to zero.
