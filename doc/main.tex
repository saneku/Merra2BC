...
\texttt{--mera\_files}, \texttt{--do\_IC}/\texttt{--no\_do\_IC}, and
\texttt{--do\_BC}/\texttt{--no\_do\_BC}. Use ``python \textit{main.py} --help'' to print all options.
\item \textit{real.exe} sets default boundary and initial conditions for some chemical species.  {Merra2BC} adds interpolated values to the existing values, which may cause incorrect concentration values. To avoid this, run ``python \textit{zero\_fields.py}'', which will zero the required fields.
...
``python \textit{main.py} --wrf\_dir /path/to/wrf/run --wrf\_met\_dir /path/to/wps/run --mera\_dir /path/to/merra --do\_IC --do\_BC''.
