...
wrf_out_files=parallel.get_wrfout_files(parallel.wrf_out_files_mask,pathes.wrf_dir)
print wrf_out_files
...
for file_index,wrf_out_file in wrf_out_files.items():
    nc_fid = nc.MFDataset(pathes.wrf_dir+wrf_out_file)
    times =nc_fid.variables['Times'][:]
    ...
    #    plt.show()
        print 'processing wrf_aod_%d.png'%(time_idx+parallel.number_of_hours_per_wrfout*file_index)
        plt.savefig('wrf_aod_%d.png'%(time_idx+parallel.number_of_hours_per_wrfout*file_index))
        plt.clf()
