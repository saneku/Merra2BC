...
for index in range(0,len(merra_files),6):
    ...
    avg_by=0
    sum_aod= np.zeros([merra2_module.mer_number_of_y_points,merra2_module.mer_number_of_x_points])
    for mera_time_idx in range(0,len(mera_times),1):
        ...
        #TOTAL AOD
        sum_aod = sum_aod+ merra_file.variables['TOTEXTTAU'][mera_time_idx,:,:];

        #DUST AOD
        #sum_aod = sum_aod+ merra_file.variables['DUEXTTAU'][mera_time_idx,:,:];
    ...
    #cs = ash_map.contourf(x,y,sum_aod,100,cmap=plt.cm.Spectral_r)
    ...
    cbar.set_label("Merra2 daily averaged by "+str(avg_by)+ " values total AOD, 550 nm")  
...
#        plt.title("%s on %s" % ("MERA diag:dust extinction AOT, 550nm", mera_cur_time))
    ...
    plt.savefig('merra_avg_aod_%d.png'%(findex))
    plt.clf()
    findex=findex+1
